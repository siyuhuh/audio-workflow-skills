...
- Out-of-box experience for end users. `apps/desktop/src/main/lib/bundledModels.ts` resolves two opt-in bundle folders shipped only by the Offline distribution: `vendor/separator-models/` (flat `.onnx` / `.pth` / `.ckpt` files) and `vendor/whisper-models/` (direct faster-whisper model directories). The default `pnpm dist:mac` / `dist:win` commands produce a smaller Lite installer and preserve the existing UVR-or-HF on-demand download flow. Maintainers run `apps/desktop/scripts/fetch-bundled-models.sh` before `pnpm dist:mac:offline` / `dist:win:offline` to create the full offline package (default model budget ~530 MB: `UVR-MDX-NET-Inst_HQ_3.onnx` + `Systran/faster-whisper-small`). In an Offline build, `detectAndLinkUvr()` links the bundled separator weights into the same shadow folder UVR uses, while every CLI subprocess receives `VOCALFLOW_WHISPER_MODEL_DIR` and reads the bundled Whisper model directly without duplicating a Hugging Face blob cache. `HF_HOME` remains the writable app-managed cache for optional model downloads.
