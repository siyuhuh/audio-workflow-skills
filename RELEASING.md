...
The suite marketing version for this beta is `0.8.0`; the prerelease tag and Electron semver are `0.8.0-beta.1`.

## Desktop release

```bash
git tag -a v0.8.0-beta.1 -m "VocalFlow 0.8.0 beta 1"
git push siyuhuh v0.8.0-beta.1
```
...
- First model use copies read-only bundled weights to VocalFlow's writable cache; it should not invoke pip or download the default weights.
